...
On Windows 11 with Python, Node/Corepack, Docker Desktop and package registry access, run `powershell -NoProfile -File .\scripts\Invoke-DevOrder.ps1` from the repository root. The script creates an isolated MongoDB container, installs minimal backend runtime packages and frontend dependencies, exercises local API and route smoke checks, builds the frontend, and opens the range for manual gameplay checks. It writes a JSON report and logs under your temporary directory and stops its processes afterward. A performance recording is deferred to HI-DEV-002 and is not required for this first run. See [the development order](docs/DEV_ORDER_DUAL_MODE.md) for the remaining product decisions and optimization criteria.
